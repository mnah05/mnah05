	...
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		try {
			...
			if (request.method === "GET") {
				...
				const slug = parts.length >= 3 ? decodeURIComponent(parts.slice(2).join("/")) : url.searchParams.get("slug");
				if (!isValidSlug(slug)) return json({ error: "missing or invalid slug" }, 400);
			...
			if (request.method === "POST") {
				...
				if (parts.length >= 3) slug = decodeURIComponent(parts.slice(2).join("/"));
				if (!isValidSlug(slug)) {
					try {
				...
				if (ip) {
					...
					await env.DB.prepare("INSERT OR IGNORE INTO page_views (slug, views) VALUES (?, 0)")
						.bind(slug)
						.run();
					...
					const row = await env.DB.prepare("SELECT views FROM page_views WHERE slug = ?")
						.bind(slug)
						.first<{ views: number }>();
					return json({ slug, views: row?.views ?? (counted ? 1 : 0), counted });
				}
