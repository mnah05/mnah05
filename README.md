# Hi, I'm Nauman

I'm a backend-focused engineer and computer science student based in Kolkata, India. I build small, reliable tools and systems with a focus on clean APIs, asynchronous workflows, and thoughtful infrastructure.

## What I'm building

- [Envitoo](https://envitoo.mnah.dev/) - Event management with ticketing, QR check-in, and volunteer coordination.
- [go-analytics](https://github.com/mnah05/go-analytics) - A high-performance URL shortener with real-time click analytics.
- [tripnest](https://github.com/mnah05/tripnest) - A travel planning backend with type-safe SQL and Dockerized services.

## Tools I use

`Go` `TypeScript` `Hono` `PostgreSQL` `MySQL` `Redis` `NATS JetStream` `Cloudflare Workers` `Cloudflare R2` `Docker` `Git`

## A little more

- I enjoy backend architecture, systems design, and developer tooling.
- I am currently studying Computer Science and Engineering at DIT University.
- I read, write, and keep a collection of useful references at [Raindrop](https://raindrop.io/workwithnauman/reads-72899412).

## Find me

- [Website](https://mnah.dev)
- [LinkedIn](https://www.linkedin.com/in/nauman-hasan-53a66630a/)
- [X](https://x.com/mnah05)
- [LeetCode](https://leetcode.com/u/mnah05)
- [Email](mailto:workwithnauman@gmail.com)
